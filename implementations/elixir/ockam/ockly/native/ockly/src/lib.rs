...
use ockam_identity::{
    models::{PurposeKeyAttestation, PurposePublicKey, CredentialSchemaIdentifier},
    utils::AttributesBuilder,
    Identifier, Identities, Vault,
};
use ockam_vault::{EdDSACurve25519SecretKey, HandleToSecret, SigningKeyType, SigningSecret, SigningSecretKeyHandle, SoftwareVaultForSecureChannels, SoftwareVaultForSigning, X25519PublicKey, X25519SecretKey};
...
fn create_identity(env: Env, existing_key: Option<String>) -> NifResult<(Binary, Binary)> {
    ...
    let (secret_type, existing_key) = if IDENTITY_MEMORY_VAULT.read().unwrap().is_some() {
        let existing_key = existing_key.map(|x| {
            SigningSecretKeyHandle::EdDSACurve25519(HandleToSecret::new(hex::decode(x).unwrap()))
        }); // FIXME
...
fn attest_secure_channel_key<'a>(
    ...
    let purpose_key = block_future(async move {
        let handle = secure_channel_vault
            .import_static_x25519_secret(
                X25519SecretKey::new(secret.to_vec().try_into().unwrap()), // FIXME
            )
            .await?;
        identities_ref
            .purpose_keys()
            .secure_channel_purpose_keys_creation()
            .purpose_key_builder(&identifier)
...
fn verify_secure_channel_key_attestation(
    ...
    let k = X25519PublicKey(public_key.as_slice().try_into().unwrap()); // FIXME
    block_future(async move {
...
fn import_signing_secret(secret: Binary) -> NifResult<String> {
    ...
    block_future(async move {
        let handle = signing_vault
            .import_key(
                SigningSecret::EdDSACurve25519(EdDSACurve25519SecretKey::new(
